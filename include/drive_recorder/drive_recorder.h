...
void DriveRecorder::timerCallback(const ros::TimerEvent& te)
{
  ...
  ROS_DEBUG("drive recorder timer expired (%d:%d)-(%d:%d)", 
  ...
  for( fs::directory_iterator src_file(src_directory); src_file != end; src_file++)
  {
    try{
      ...
      if( diff > 0 )
      {
        ...
        ROS_DEBUG("log file:%s -> %s mod time:%d(%d)", src_file->path().string().c_str(), dst_filename.c_str(), (int)mod_t, (int)diff );
        if( ext == ".bag" )
        {
...
void DriveRecorder::decisionMakerStateCallback(const std_msgs::String& msg)
{
  ...
  ROS_DEBUG("/decision_maker/state subscribed (%s, %d) ", msg.data.c_str(), decision_maker_state_emergency_);
}
void DriveRecorder::recordCmdCallback(const std_msgs::Header header_msg)
{
  ROS_DEBUG("drive recorder subscribed (%d:%d) ", header_msg.stamp.sec, header_msg.stamp.nsec);
  startTimer();
}
...
void DriveRecorder::startTimer()
{
  ...
  {
    ROS_DEBUG("timer already started!!!");
  }
}

void DriveRecorder::stopRequested()
{
  ROS_DEBUG("stop requedted");
  startTimer();
}
...
DriveRecorder::DriveRecorder() : private_nh_("~")
{
  ...
  ROS_DEBUG("%d %d %d %s %s", _before, _after, _bag_period, src_dirname_.c_str(), dst_dirname_.c_str());
